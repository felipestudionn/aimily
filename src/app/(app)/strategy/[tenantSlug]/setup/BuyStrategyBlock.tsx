...
export function BuyStrategyBlock({
  ...
  const handleSelectArchetype = useCallback(
    async (archetype: BuyStrategyArchetype) => {
      ...
      setError('');
      try {
        const res = await fetch('/api/strategy/buy-strategy-prefill-editor', {
          method: 'POST',
  ...
  const handleDeepen = useCallback(
    async (axis: string) => {
      ...
      try {
        ...
        const res = await fetch('/api/strategy/buy-strategy-deepen', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            tenant_id: tenant.id,
  ...
  const handleConfirm = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/strategy/buy-strategy-confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tenant_id: tenant.id,
      ...
      router.refresh();
      if (data.run_id) {
        router.push(`/strategy/${tenant.slug}/runs/${data.run_id}`);
      } else {
        router.push(`/strategy/${tenant.slug}`);
      }
    } catch (err) {
