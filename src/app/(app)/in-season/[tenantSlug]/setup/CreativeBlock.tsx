 ...
 *
 * Layout (top → bottom):
 *   1. Market trends · auto-loaded from /api/in-season/market-trends, grouped
 ...
 *   3. Confirm CTA · synthesises brief via /api/in-season/briefs/discover
 *      and persists to /api/in-season/briefs.
 */
...
export function CreativeBlock({ tenant, existingBrief: _existingBrief, gatingBlocked, onSaved }: Props) {
  ...
  const loadMarketTrends = useCallback(async () => {
    ...
    setTrendsError('');
    try {
      const res = await fetch('/api/in-season/market-trends', {
        method: 'POST',
  ...
  const handleAnalyzeMoodboard = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/in-season/analyze-moodboard', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/in-season/briefs/discover', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const saveRes = await fetch('/api/in-season/briefs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tenant_id: tenant.id,
