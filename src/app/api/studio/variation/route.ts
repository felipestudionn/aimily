...
function buildVariationPrompt(body: VariationBody, newModel: AimilyModel | null): string {
  ...
  return [
    ...
    `HEAD ANGLE: the head's angle, tilt, and gaze should flow naturally with the body's pose in Image 1 — if the shoulders twist or the arms reach, the head turns or tilts organically. The head must read as part of the body's movement, not as a portrait sitting on top of a moving body.`,
