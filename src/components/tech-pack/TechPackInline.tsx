...
/* Multi-view drawings hint — empty by default; we populate `extraSlots`
   with a couple of category-relevant labels so the user sees the option. */
function suggestedExtraSlots(category?: string): Array<{ slot: string; label: string }> {
  if (category === 'CALZADO') return [{ slot: 'viewC', label: 'Heel detail' }, { slot: 'viewD', label: 'Sole pattern' }];
  if (category === 'ROPA') return [{ slot: 'viewC', label: 'Back view' }, { slot: 'viewD', label: 'Detail / closure' }];
  return [];
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const preferredViewB = sku.sketch_top_url || '';
